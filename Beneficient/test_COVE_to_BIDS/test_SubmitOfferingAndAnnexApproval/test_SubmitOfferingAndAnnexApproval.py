...
def test_setup():
  ...
  TestName = "test_SubmitOfferingAndAnnexApproval"
  description = "This test scenario is to verify presence of elements inside Submit Offering and Annex A Approval"
  TestResult = []
  TestResultStatus = []
  TestFailStatus = []
  FailStatus="Pass"
...
def test_PrepareLiquidityRequest(test_setup):
    if Exe == "Yes":
        try:
            ...
            # ----------------------------------------------------------------------------------------
            PageName = "Submit Offering and Annex A Approval"
            ii = 17
            try:
                driver.find_element_by_xpath(
