...
def test_setup():
  ...
  TestName = "test_CreateScenarios"
  description = "This test scenario is to verify presence of elements inside Create Scenarios"
  TestResult = []
  TestResultStatus = []
  TestFailStatus = []
  FailStatus="Pass"
...
def test_PrepareLiquidityRequest(test_setup):
    if Exe == "Yes":
        try:
            ...
            # ---------------------------------ALL SCENARIOS CREATED button present in page-------------------------------
            Text1 = "ALL SCENARIOS CREATED"
            Element1 = driver.find_element_by_xpath(
                "//div[@class='ContentLayout---content_layout']/div[4]/div[2]/div/div[2]/div/div[4]/button").text
            try:
