...
def test_setup():
  ...
  description = "This is regression test case to verify presence of all currencies in list at SOS Funds"
  TestResult = []
  TestResultStatus = []
  TestFailStatus = []
  FundsNamesList = []
...
def test_CurrencyListSoSFunds(test_setup):
    if Exe == "Yes":
        try:
            ...
            Ptitle1 = driver.find_element_by_xpath("//div[@class='ContentLayout---content_layout']/div[1]/div/div[3]/div[1]/div/div[2]/div/div/div[2]/div/div[6]/div/div[2]/div/div/span").text
            start = time.time()
            try:
                WebDriverWait(driver, SHORT_TIMEOUT
                              ).until(EC.presence_of_element_located((By.XPATH, LOADING_ELEMENT_XPATH)))
            ...
            for ii1 in range (1,18):
                ...
                if ii1>1:
                    if Ptitle1 in "Select Currency":
                        ...
                        break
                driver.find_element_by_xpath("//div[@class='ContentLayout---content_layout']/div[1]/div/div[3]/div[1]/div/div[2]/div/div/div[2]/div/div[6]/div/div[2]/div/div").click()
                time.sleep(1)
                ...
                try:
                    ...
                    if bool1 == True:
                        ...
                        driver.find_element_by_xpath(
                            "//div[@class='appian-context-ux-responsive']/div[4]/div/div/div[2]/div/button").click()
                        TestResult.append("SoS Funds page not able to open\n" + ErrorFound1)
                        TestResultStatus.append("Fail")
                        bool1 = False
                except Exception:
                    try:
            ...
            if len(ListOfCountries)>=17:
                ...
                TestResult.append("SoS Funds page has all currencies in list as given below\n"+ListC)
                TestResultStatus.append("Pass")
            else:
                print("Not all Currencies available")
                TestResult.append("SoS Funds page has less currencies in list\n Page has total "+str(len(ListOfCountries))+" currencies")
                TestResultStatus.append("Fail")
