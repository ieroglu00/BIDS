...
def test_setup():
  ...
  TestName = "test_CurrenciesSoSInvestments"
  description = "This is regression test case to verify presence of all currencies in list at SOS Investments"
  TestResult = []
  TestResultStatus = []
  TestFailStatus = []
  FundsNamesList = []
...
def test_CurrencyListSoSInvestments(test_setup):
    if Exe == "Yes":
        try:
            ...
            # ---------------------------Verify Quarterly NAV Close page-----------------------------
            ...
            # ---------------------------Verify Sign-Off Summary: Funds page-----------------------------
            PageName = "Sign-Off Summary: Investments"
            PageTitle = "User Input Task - BIDS"
            ...
            # --------------------Clicking on Select Currency--------------
            PageName = "Select Currency"
            Ptitle1 = driver.find_element_by_xpath("//div[@class='ContentLayout---content_layout']/div[2]/div/div/div/div[2]/div[1]/div/div[2]/div/div/div[2]/div/div[6]/div/div[2]/div/div/span").text
            start = time.time()
            try:
            ...
            time.sleep(1)
            try:
                assert PageName in Ptitle1, PageName + " not able to open"
            ...
            #----------------Loop to get all currencies-------------------
            for ii1 in range (1,18):
                print(ii1)
                Ptitle1 = driver.find_element_by_xpath("//div[@class='ContentLayout---content_layout']/div[2]/div/div/div/div[2]/div[1]/div/div[2]/div/div/div[2]/div/div[6]/div/div[2]/div/div/span").text
                if ii1>1:
                    if Ptitle1 in "Select Currency":
                        print("Ending loop "+Ptitle1)
                        break
                driver.find_element_by_xpath("//div[@class='ContentLayout---content_layout']/div[2]/div/div/div/div[2]/div[1]/div/div[2]/div/div/div[2]/div/div[6]/div/div[2]/div/div").click()
                time.sleep(1)
                ...
                try:
                    ...
                    if bool1 == True:
                        ...
                        driver.find_element_by_xpath(
                            "//div[@class='appian-context-ux-responsive']/div[4]/div/div/div[2]/div/button").click()
                        TestResult.append("SoS Investments page not able to open\n" + ErrorFound1)
                        TestResultStatus.append("Fail")
                        bool1 = False
                except Exception:
                    try:
                    ...
                    pass
                print(Ptitle1)
                ListOfCountries.append(Ptitle1)
            print(str(len(ListOfCountries)))
            ListC=', '.join(ListOfCountries)
            print(ListC)
            if len(ListOfCountries)>=17:
                print("All Currencies available")
                TestResult.append("SoS Investments page has all currencies in list as given below\n"+ListC)
                TestResultStatus.append("Pass")
            else:
                print("Not all Currencies available")
                TestResult.append("SoS Investments page has less currencies in list\n Page has total "+str(len(ListOfCountries))+" currencies")
                TestResultStatus.append("Fail")
