...
@pytest.mark.smoke
def test_ReportSendValuesCheck():
    print()
    DirectoryName = []
    PDFName1=[]
    TestName=[]
    ...
    html = '''
        <html>
            <body>
                <p>Hi Team <br />Here is the test summary report of Smoke Test 3 (To verify values in all modules) <br />Below test scenarios are covered </p>
                <p></p>
                <p>'''+B+'''</p
                <p></p>
    ...
    msg = MIMEMultipart()
    msg['Subject']='Test Automation Report [Smoke Test 3] -Env [Test] '+date_str
    msg['From'] = email_from
    ...
    #-----------------------------------------------------------------------
    try:
        attach_file_to_email(msg, path+'/TestPieResult.png',
    ...
    for file in PDFName1:
        ...
        try:
            ...
            if SendStatus[i]=="Send Only when Fail=Yes" and  TestStatus[i]=="Fail":
                    attach_file_to_email(msg,path+PDFName1[i])
    ...
    for ii in range(0,len(PDFName1)):
        print()
        try:
            os.remove(path+PDFName1[ii])
        except Exception:
            print("No Attachment found to delete")
    try:
        os.remove(path+'TestPieResult.png')
    except Exception:
        print("No Attachment found to delete")
    #-----------------------------------------------------------------------
    server.quit()
